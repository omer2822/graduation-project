...
import ScheduleConfirmationDialog from '../components/scheduleConfirmationDialog/ScheduleConfirmationDialog ';
            ...
            try {
                ...
                const employees = response.data.map(emp => {
                  const skills = ['Skill1', 'Skill2', 'Skill3']
                    .map(skillKey => emp[skillKey])
                    .filter(skill => skill && skill.trim() !== '');
                
                  const { Skill1, Skill2, Skill3, ...rest } = emp;
                  return { ...rest, skills };
                });
    ...
    const handleSchedule = async () => {
        try {
            ...
            if (response.status === 200) {
                setScheduledShifts(response.data.assignments || []);
    ...
    return (
        <Container>
            ...
            <ScheduleConfirmationDialog
                ...
                employees={employees}
                setScheduledShifts={setScheduledShifts}
